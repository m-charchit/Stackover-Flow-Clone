...
def get_img(img):
    return storage.child(img).get_url(firebase_user["idToken"]) if img \
        != "default.jpg" else url_for('static', filename='images/default.jpg')
...
def ques_page(sno, slug):
    ...
    if "user" in session and session["loggedin"] is True:
        # allow user to answer,comment,edit,delete if loggedin
        ...
        if real is not None:
            ...
            if my_answer.username == session["user"]:
                ...
                if len(Answers.query.filter_by(real_answer="true").all()) <= 1:
                    return "success"  # adding the true or false and returning success,
                my_answer.real_answer = before_request
                db.session.commit()
            return "If you do this again. You will be banned from site."\
                + session["user"]

        if tab == "oldest":
            answers = Answers.query.filter_by(ques_id=sno).order_by(
                Answers.real_answer.desc(), Answers.date).all()  # sorting by first real_answer so if it is oldest 
